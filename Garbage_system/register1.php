<?php
    ...
    $err = '';
    if(isset($_POST['btn1'])){
        $sql = 'SELECT * FROM address1 WHERE name_garbage="'.$_POST['name_garbage'].'"';
        $rs = mysqli_query($GLOBALS['conn'],$sql);
        if(mysqli_num_rows($rs)!=0){$err = '<div class="alert alert-danger">ชื่อผู้ใช้นี้มีอยู่แล้ว</div>';}

        if($_POST['name_garbage']=="admin"){$err = '<div class="alert alert-danger">ไม่สามารถใช้ชื่อนี้ได้</div>';}

        if($err == ''){
            $sql = 'INSERT INTO address1 (name_garbage,password,name,est,tel,mail,house_no,village_no,alley,road,sub_area,district,Province,postal_code,latitude,longitude)
            VALUES ("'.$_POST['name_garbage'].'",
                    "'.md5($_POST['password']).'",
                    "'.$_POST['name'].'",
                    "'.$_POST['est'].'",
                    "'.$_POST['tel'].'",
...
<div class="wrapper">
    <div class="form-box register">
        ...
        </div><button name="btn2"  class="btn" >กรอกละติจูด/ลองติจูดอัตโนมัติ</button>
        <br><br>
